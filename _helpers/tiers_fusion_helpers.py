...
class TiersFusionHelpers(BaseHelper):
    def __init__(self, services=None):
        ...
        if not self.tiers_trackers:
            raise ValueError("Tracker tiers non trouvé dans services")
        objet_vide = self.tiers_trackers.get_new()
    ...
    def load_data(self, selected_tiers_data: list):
        ...
        for row in selected_tiers_data:
            ...
            if t_id is not None:
                obj_complet = self.tiers_trackers.get_by_id(t_id)
                if obj_complet:
                    self.tiers_list.append(obj_complet)

        print(f"[DEBUG] Helper : {len(self.tiers_list)} objets métiers chargés.")

        # --- AJOUT ICI ---
        # Une fois les vrais objets chargés, on extrait les champs du premier
        if self.tiers_list:
            self.champs_a_comparer = self._extraire_champs(self.tiers_list[0])
    ...
    def get_columns_config(self):
        ...
        for i, t in enumerate(self.tiers_list):
    ...
    _SQL_TO_ATTR = {
        "":       None,   # FK gérée via l'objet .banque, non éditable ici
    }
    ...
    def prepare_rows(self):
        ...
        for champ_sql in self.champs_a_comparer:
            ...
            valeurs_tiers = [str(getattr(t, attr, "")) for t in self.tiers_list]
    ...
    def executer_fusion(self, resultats_apres: dict):
        ...
        if not self.tiers_list:
            return

        champs_maitre = self.tiers_list[0]
        id_maitre = champs_maitre.id_tiers
        ...
        success = self.tiers_trackers.update(champs_maitre)
        print(f"[DEBUG] compte_tracker.update => {success}")

        # 2. Identification des doublons à supprimer
        ids_doublons = [t.id_tiers for t in self.tiers_list[1:] if t.id_tiers != id_maitre]
        ...
        if ops_tracker:
            nb = ops_tracker.migrer_liaisons_tiers(ids_doublons, id_maitre)
        ...
        for old_id in ids_doublons:
            deleted = self.tiers_trackers.delete(old_id)
    ...
    def _extraire_champs(self, objet_vide) -> list:
        ...
        if hasattr(objet_vide, 'to_sql_dict'):
            ...
            return [k for k in dico.keys() if k != 'id_tiers']